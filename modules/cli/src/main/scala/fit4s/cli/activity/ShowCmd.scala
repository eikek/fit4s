...
object ShowCmd extends SharedOpts with FormatDefinition {
  ...
  val opts: Opts[Options] = {
    ...
    (id, outputFormatOpts).mapN(Options.apply)
  }

  def apply(cliCfg: CliConfig, opts: Options): IO[ExitCode] =
    activityLog(cliCfg).use { log =>
      for {
        result <- log.activityDetails(opts.id, false)
        _ <- result match {
          case Some(r) =>
            opts.format.fold(
              IO.println(
  ...
  private def showLaps(laps: List[ActivityLap])(implicit zoneId: ZoneId): String =
    if (laps.isEmpty) ""
    else {
      val labels =
        List(
  ...
  private def distanceLocationAndTime(r: ActivityDetailResult, s: ActivitySession) = {
    val fromTo =
  ...
  private def avgMaxTable(s: ActivitySession): String = {
    implicit val sport: Sport = s.sport
    val data = List(
      ("", "Avg", "Max"),
  ...
  private def propertiesTable(s: ActivitySession): String = {
    val data = List(
  ...
  private def filePath(r: ActivityDetailResult): String = {
    val path = r.location.locationPath / r.activity.path
