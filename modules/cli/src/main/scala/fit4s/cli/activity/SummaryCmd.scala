...
object SummaryCmd extends SharedOpts {
  ...
  val opts: Opts[Options] =
    (activitySelectionOps, pageOpts, outputFormatOpts).mapN(Options.apply)
  ...
  object StartEnd {
    ...
    implicit def jsonEncoder[A: Encoder]: Encoder[StartEnd[A]] =
      Encoder { (w, r) =>
        w.writeMapStart()
        w.writeMapMember("start", r.start)
        w.writeMapMember("end", r.end)
        w.writeMapClose()
      }
  }
  ...
  object Summary {
    ...
    implicit def jsonEncoder(implicit zoneId: ZoneId, sport: Sport): Encoder[Summary] = {
      import fit4s.common.borer.DataJsonCodec._
