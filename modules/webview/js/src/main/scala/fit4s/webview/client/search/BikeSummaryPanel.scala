...
import _root_.fit4s.activities.data.ActivitySessionSummary
import _root_.fit4s.activities.data.Tag
import _root_.fit4s.cats.instances.all.*
import _root_.fit4s.profile.types.Sport
import _root_.fit4s.webview.client.cmd.*
import _root_.fit4s.webview.client.shared.{ErrorPanel, Styles}
import calico.*
...
object BikeSummaryPanel {
  ...
  def subscribe(model: SignallingRef[IO, Model], cr: CommandRuntime[IO]) =
    cr.subscribe.evalMap {
      case Result.TagSummaryResult(result) =>
        val data = result.fold(
          _.filter(_._1.name.startsWith1("Bike/"))
            .flatMap(t => t._2.find(_.sport == Sport.Cycling).map(r => t._1 -> r)),
          err => {
