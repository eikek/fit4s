  ...
  def from(lapMsg: DataMessage): Either[String, FitActivityLap] =
    if (!lapMsg.isMessage(LapMsg)) Left(s"Not a session message: $lapMsg")
      ...
      for {
        ...
        maxSpeed1 <- lapMsg.getField(LapMsg.enhancedMaxSpeed)
        maxSpeed2 <- lapMsg.getField(LapMsg.maxSpeed)
        maxSpeed = maxSpeed1.orElse(maxSpeed2)
        avgSpeed1 <- lapMsg.getField(LapMsg.enhancedAvgSpeed)
        avgSpeed2 <- lapMsg.getField(LapMsg.avgSpeed)
